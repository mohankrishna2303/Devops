...
class DatabaseService {
  ...
  async updateProject(id, projectData) {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000/api'}/projects/${id}/`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  async createPipeline(pipelineData) {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000/api'}/pipelines/`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  async provisionInfrastructure(infrastructureData) {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000/api'}/terraform/apply/`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  async connectProvider(provider, credentials) {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000/api'}/integrations/`, {
        method: 'GET'
      });
  ...
  async logAudit(action, resourceType, resourceId, details) {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000/api'}/analytics/dashboard/`, {
        method: 'GET'
      });
    } catch (error) {
