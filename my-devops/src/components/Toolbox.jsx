...
// ──────────────────────────────────────────────────────────────────────────────
// The actual terraform/main.tf from the project folder, rendered in-browser
// ──────────────────────────────────────────────────────────────────────────────
const TERRAFORM_MAIN_TF = `# BrainDevOps – terraform/main.tf
# This file is LIVE inside your devops/terraform/ folder.
...
# VPC — isolates all platform services
resource "aws_vpc" "main" {
  cidr_block           = "10.0.0.0/16"
  enable_dns_hostnames = true
  tags = { Name = "braindevops-vpc", Env = var.environment }
}

# EKS Cluster — hosts backend, frontend containers
...
# S3 — stores CI/CD pipeline artifacts
resource "aws_s3_bucket" "artifacts" {
  bucket = "braindevops-artifacts-\${var.environment}-\${random_id.suffix.hex}"
  tags   = { Name = "Pipeline Artifacts", Environment = var.environment }
}`;

const TERRAFORM_VARIABLES_TF = `# terraform/variables.tf
...
const TERRAFORM_OUTPUTS_TF = `# terraform/outputs.tf
...
const Toolbox = () => {
    ...
    const SyncBadge = ({ status }) => {
        ...
        const colors = { syncing: '#FFB74D', done: '#00E676', error: '#FF3D00' };
    ...
    return (
        <div className="fade-in">
                                    ...
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px' }}>
                                        <div>
                                            ...
                                            <div style={{ display: 'flex', gap: '12px', fontSize: '12px', color: 'var(--text-muted)' }}>
                                                ...
                                                <span style={{ color: plan.status === 'Applied' ? '#00E676' : 'var(--primary)' }}>● {plan.status}</span>
                                            </div>
                                        </div>
                                        ...
                                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '10px' }}>
                                            {plan.resources.map((res, j) => (
                                                <div key={j} style={{ padding: '10px', background: 'rgba(0,0,0,0.2)', borderRadius: '8px' }}>
                                                    ...
                                                    <span style={{ fontSize: '10px', color: res.status === 'Healthy' ? '#00E676' : 'var(--primary)' }}>● {res.status}</span>
                                                </div>
                                            ))}
                                        </div>
                                    )}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                    <div className="glass-card" style={{ padding: '24px' }}>
                        <h4 style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px' }}>
                            <Container size={20} color="#00E676" /> K8s Fleet
                        </h4>
                        {k8sFleet.length > 0 ? k8sFleet.map((cluster, i) => (
                            <div key={i} style={{ padding: '14px', background: 'rgba(255,255,255,0.02)', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.05)', marginBottom: '12px' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                                    ...
                                    <span style={{ fontSize: '11px', color: '#00E676', fontWeight: 'bold' }}>{cluster.status}</span>
                                </div>
            ...
            <div className="glass-card" style={{ padding: '28px', marginBottom: '32px', border: '1px solid rgba(255,61,0,0.2)' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '200px 1fr', gap: '20px' }}>
                    ...
                    <div style={{ background: 'rgba(0,0,0,0.3)', borderRadius: '12px', padding: '16px' }}>
                        ...
                        {Object.keys(tfFiles).map(f => (
                            <button key={f} onClick={() => setActiveTfFile(f)} style={{
                                ...
                                background: activeTfFile === f ? 'rgba(255,61,0,0.15)' : 'transparent',
                                border: activeTfFile === f ? '1px solid rgba(255,61,0,0.4)' : '1px solid transparent',
                    ...
                    <div>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                            ...
                            <span style={{ fontSize: '11px', color: 'var(--text-muted)', background: 'rgba(0,230,118,0.1)', color: '#00E676', padding: '2px 10px', borderRadius: '10px' }}>● Live File</span>
                        </div>
                        <pre style={{
                            background: '#0A0A0F',
                            ...
                            <div style={{ background: 'rgba(0,0,0,0.3)', borderRadius: '16px', padding: '24px' }}>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                                    {[
                                        ...
                                        { step: 'Monitor', desc: `Watch status change in real-time across Dashboard, Environments & K8s tabs`, color: '#00E676' },
            ...
            <div style={{ padding: '28px', background: 'rgba(255,255,255,0.02)', borderRadius: '20px', border: '1px solid var(--glass-border)' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '20px' }}>
                    {[
                        { icon: <Search size={20} color="var(--primary)" />, bg: 'rgba(255,61,0,0.1)', title: 'Log Pattern Analyzer', desc: 'Scan millions of log lines to find anomalies and root causes.', btn: 'Open Analyzer' },
                        { icon: <ShieldCheck size={20} color="#00E676" />, bg: 'rgba(0,230,118,0.1)', title: 'Secret Leak Scanner', desc: 'Deep scan all connected repos for exposed API keys or credentials.', btn: 'Start Audit' },
